...
//--- Read the ADC result from IPC1
	*AdcBufPtr++ = (Uint16) IpcRegs.IPCRECVDATA;    // Read the result
...
//--- Write next data point from sine table
	SineData = ((QuadratureTable[iQuadratureTable++] ^ 0x8000) >> 5);
...
//--- Example: Toggle GPIO31 at a 0.1 sec rate (connected to the LED).
//             (1/50000 sec/sample)*(1 samples/int)*(x interrupts/toggle) = (0.1 sec/toggle)
//             ==> x = 5000
	if(GPIO31_count++ > 5000)                      // Toggle slowly to see the LED blink
	{
